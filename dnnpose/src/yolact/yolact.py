...
# Modification was done to perform fine tuning (line #490 & #634): https://github.com/dbolya/yolact/issues/454
        ...
        for key in list(state_dict.keys()):
            ...
            if key.startswith('fpn.downsample_layers.'):
                if cfg.fpn is not None and int(key.split('.')[2]) >= cfg.fpn.num_downsample:
                    del state_dict[key]
        # Change from self.load_state_dict(state_dict) to below for fine tuning:
        try:
    ...
    def forward(self, x):
        ...
        with timer.env('pred_heads'):
            ...
            for idx, pred_layer in zip(self.selected_layers, self.prediction_layers):
                ...
                p = pred_layer(pred_x)
                # p = pred_layer(pred_x.detach()) This detach other deeper layers from sotf-max classification layer
